...
var _ messagerContainerProvider = (*ContainerHub)(nil)
...
func (h *ContainerHub) Load(dir string, format format.Format, options ...load.Option) error {
	...
	for name, msger := range messagerMap {
		if err := msger.ProcessAfterLoadAll(tmpHub.Hub); err != nil {
...
func (h *ContainerHub) ProvidedBy(ctx context.Context) *ProvidedHub {
	hub := &ProvidedHub{}
	hub.Hub = h.Hub.providedBy(hub)
	hub.providedCtx = ctx
	hub.baseProvider = h
	return hub
}

var _ messagerContainerProvider = (*ProvidedHub)(nil)
