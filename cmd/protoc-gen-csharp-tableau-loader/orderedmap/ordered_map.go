	...
	for i := 0; i < md.Fields().Len(); i++ {
		...
		if fd.IsMap() {
			...
			if nextMapFD != nil {
				...
				x.g.P(helper.Indent(2), "public class ", orderedMapValue, " : Tuple<", nextOrderedMap, ", ", currValueType, ">")
...
func (x *Generator) genOrderedMapLoader(md protoreflect.MessageDescriptor, depth int) {
	for i := 0; i < md.Fields().Len(); i++ {
		...
		if fd.IsMap() {
			...
			x.g.P(helper.Indent(depth+2), "foreach (var (", keyName, ", ", valueName, ") in ", prevContainer, ".", helper.ParseCsharpPropertyName(fd), ")")
	...
	for i := 0; i < md.Fields().Len(); i++ {
		...
		if fd.IsMap() {
			...
			if depth == 1 {
				...
				x.g.P(helper.Indent(2), "/// ", getter, " returns the ", loadutil.Ordinal(depth), "-level ordered map.")
				x.g.P(helper.Indent(2), "/// </summary>")
				x.g.P(helper.Indent(2), "public ref readonly ", orderedMap, " ", getter, "() => ref _orderedMap;")
			} else {
				x.g.P(helper.Indent(2), "/// <summary>")
				x.g.P(helper.Indent(2), "/// ", getter, " finds value in the ", loadutil.Ordinal(depth-1), "-level ordered map.")
				x.g.P(helper.Indent(2), "/// It will return null if the key is not found.")
				x.g.P(helper.Indent(2), "/// </summary>")
