...
func (x *Generator) genIndexLoader() {
	...
	for levelMessage := x.descriptor.LevelMessage; levelMessage != nil; levelMessage = levelMessage.NextLevel {
		for _, index := range levelMessage.Indexes {
			x.genOneIndexLoader(levelMessage.MapDepth, levelMessage.Depth, index, parentDataName)
		}
...
func (x *Generator) genOneIndexLoader(depth int, ident int, index *index.LevelIndex, parentDataName string) {
	...
	if len(index.ColFields) == 1 {
		...
		if field.FD.IsList() {
			...
			key := itemName
			x.genLoader(depth, ident+4, index, key, parentDataName)
			x.g.P(helper.Indent(ident+3), "}")
		} else {
			key := parentDataName + fieldName
			x.g.P(helper.Indent(ident+3), "var key = ", key, ";")
			x.genLoader(depth, ident+3, index, "key", parentDataName)
		}
	} else {
		// multi-column index
		x.generateOneMulticolumnIndex(depth, ident+2, index, parentDataName, nil)
	}
	x.g.P(helper.Indent(ident+2), "}")
}

func (x *Generator) generateOneMulticolumnIndex(depth, ident int, index *index.LevelIndex, parentDataName string, keys helper.MapKeySlice) {
	cursor := len(keys)
	...
	if field.FD.IsList() {
		...
		x.generateOneMulticolumnIndex(depth, ident+1, index, parentDataName, keys)
		x.g.P(helper.Indent(ident+1), "}")
	} else {
		key := parentDataName + fieldName
		keys = keys.AddMapKey(helper.MapKey{Name: key})
		x.generateOneMulticolumnIndex(depth, ident, index, parentDataName, keys)
	}
}
