...
func genHppOrderedMapGetters(g *protogen.GeneratedFile, md protoreflect.MessageDescriptor, depth int, keys []helper.MapKey, messagerFullName string) {
	...
	for i := 0; i < md.Fields().Len(); i++ {
		...
		if fd.IsMap() {
			if depth == 1 {
				g.P(helper.Indent(1), "// OrderedMap accessers.")
				g.P(" public:")
			}
			...
			if nextMapFD != nil {
				...
				g.P(helper.Indent(1), "using ", orderedMapValue, " = std::pair<", nextOrderedMap, ", const ", currValueType, "*>;")
				g.P(helper.Indent(1), "using ", orderedMap, " = std::map<", keyType, ", ", orderedMapValue, ">;")
				g.P(helper.Indent(1), "const ", orderedMap, "* GetOrderedMap(", helper.GenGetParams(keys), ") const;")
				g.P()
			} else {
				g.P(helper.Indent(1), "using ", orderedMap, " = std::map<", keyType, ", ", parseMapValueType(fd), ">;")
				g.P(helper.Indent(1), "const ", orderedMap, "* GetOrderedMap(", helper.GenGetParams(keys), ") const;")
				g.P()
			}
			if depth == 1 {
				g.P(" private:")
				g.P(helper.Indent(1), orderedMap, " ordered_map_;")
			}
			break
		}
	}
...
func genCppOrderedMapLoader(g *protogen.GeneratedFile, md protoreflect.MessageDescriptor, depth int, messagerFullName string) {
	if depth == 1 {
		g.P(helper.Indent(1), "// OrderedMap init.")
		g.P(helper.Indent(1), "ordered_map_.clear();")
	}
	for i := 0; i < md.Fields().Len(); i++ {
		fd := md.Fields().Get(i)
		if fd.IsMap() {
			...
			g.P(helper.Indent(depth), "for (auto&& ", itemName, " : ", prevContainer, ".", string(fd.Name()), "()) {")
			nextMapFD := getNextLevelMapFD(fd.MapValue())
			if nextMapFD != nil {
				nextPrefix := parseOrderedMapPrefix(nextMapFD, messagerFullName)
				// nextMap := nextPrefix + mapSuffix
				nextOrderedMap := nextPrefix + orderedMapSuffix
				g.P(helper.Indent(depth+1), prevTmpOrderedMapName, "[", itemName, ".first] = ", orderedMapValue, "(", nextOrderedMap, "(), &", itemName, ".second);")
				g.P(helper.Indent(depth+1), "auto&& ", tmpOrderedMapName, " = ", prevTmpOrderedMapName, "[", itemName, ".first].first;")
			} else {
				ref := "&"
				if fd.MapValue().Kind() != protoreflect.MessageKind {
					ref = "" // scalar value type just do value copy.
				}
				g.P(helper.Indent(depth+1), prevTmpOrderedMapName, "[", itemName, ".first] = ", ref, itemName, ".second;")
			}
			if fd.MapValue().Kind() == protoreflect.MessageKind {
...
func genCppOrderedMapGetters(g *protogen.GeneratedFile, md protoreflect.MessageDescriptor, depth int, keys []helper.MapKey, messagerName, messagerFullName string) {
	...
	for i := 0; i < md.Fields().Len(); i++ {
		...
		if fd.IsMap() {
			...
			if depth == 1 {
				g.P(helper.Indent(1), "return &ordered_map_; ")
			} else {
				lastKeyName := keys[len(keys)-1].Name
				prevKeys := keys[:len(keys)-1]
				g.P(helper.Indent(1), "const auto* conf = GetOrderedMap(", helper.GenGetArguments(prevKeys), ");")
				g.P(helper.Indent(1), "if (conf == nullptr) {")
				g.P(helper.Indent(2), "return nullptr;")
				g.P(helper.Indent(1), "}")
				g.P(helper.Indent(1), "auto iter = conf->find(", lastKeyName, ");")
				g.P(helper.Indent(1), "if (iter == conf->end()) {")
				g.P(helper.Indent(2), "return nullptr;")
				g.P(helper.Indent(1), "}")
				g.P(helper.Indent(1), "return &iter->second.first;")
