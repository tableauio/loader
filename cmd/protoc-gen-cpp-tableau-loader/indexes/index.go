		...
		for _, index := range levelMessage.Indexes {
			...
			for i := 1; i <= levelMessage.Depth-2; i++ {
				...
				if i == 1 {
					...
					x.g.P(helper.Indent(1), "std::unordered_map<", leveledIndexKeyType, ", ", mapType, ", ", leveledIndexKeyType, "Hasher> ", x.indexContainerName(index, i), ";")
				}
			}
		}
	}
...
func (x *Generator) genLoader(depth, ident int, index *index.LevelIndex, key, parentDataName string) {
	...
	for i := 1; i <= depth-2; i++ {
		...
		if i == 1 {
			x.g.P(helper.Indent(ident), x.indexContainerName(index, i), "[item1.first][", key, "].push_back(&", parentDataName, ");")
		} else {
			var fields []string
			for j := 1; j <= i; j++ {
