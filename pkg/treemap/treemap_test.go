...
func TestMapMin(t *testing.T) {
	...
	if k, v, ok := m.Min(); ok {
		t.Errorf("Got %v->%v expected empty map", k, v)
	}
...
func benchmarkGet[V any](b *testing.B, m *TreeMap[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Get(n)
		}
...
func benchmarkPut[V any](b *testing.B, m *TreeMap[int, V], size int) {
	var value V
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Put(n, value)
...
func benchmarkRemove[V any](b *testing.B, m *TreeMap[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Remove(n)
		}
