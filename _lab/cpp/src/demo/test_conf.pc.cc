...
bool ActivityConf::ProcessAfterLoad() {
  ...
  for (auto&& item1 : data_.activity_map()) {
    ...
    for (auto&& item2 : item1.second.chapter_map()) {
      ordered_map1[item2.first] = Activity_Chapter_OrderedMapValue(protoconf_Section_OrderedMap(), &item2.second.section_map());
      auto&& ordered_map2 = ordered_map1[item2.first].first;
      for (auto&& item3 : item2.second.section_map()) {
        ordered_map2[item3.first] = protoconf_Section_OrderedMapValue(int32_OrderedMap(), &item3.second.section_rank_map());
...
const int32_t* ActivityConf::Get(uint64_t key1, uint32_t key2, uint32_t key3, uint32_t key4) const {
  const auto* conf = Get(key1, key2, key3);
  if (conf == nullptr) {
    return nullptr;
  }
...
const ActivityConf::protoconf_Section_OrderedMap* ActivityConf::GetOrderedMap(uint64_t key1, uint32_t key2) const {
  const auto* conf = GetOrderedMap(key1);
  if (conf == nullptr) {
    return nullptr;
  }
...
const ActivityConf::int32_OrderedMap* ActivityConf::GetOrderedMap(uint64_t key1, uint32_t key2, uint32_t key3) const {
  const auto* conf = GetOrderedMap(key1, key2);
  if (conf == nullptr) {
    return nullptr;
  }

  auto iter = conf->find(key3);
  if (iter == conf->end()) {
    return nullptr;
  }
  return &iter->second.first;
...
const ActivityConf::Index_NamedChapterVector* ActivityConf::FindNamedChapter(std::string chapter_name) const {
...
const protoconf::ActivityConf::Activity::Chapter* ActivityConf::FindFirstNamedChapter(std::string chapter_name) const {
...
const protoconf::ChapterConf::Chapter* ChapterConf::Get(uint64_t key1) const {
  auto iter = data_.chapter_map().find(key1);
...
const protoconf::ThemeConf::Theme* ThemeConf::Get(std::string key1) const {
  auto iter = data_.theme_map().find(key1);
